class FactoryScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(width / 2 + 20, 56, 'FACTORY FLOOR', {
      fontFamily: 'monospace',
      fontSize: '20px',
      color: '#eef2f8',
      fontStyle: 'bold'
    ...
    this.add.text(width / 2 + 20, 82, 'PLACE MACHINES · TAP TO BUILD', {
      fontFamily: 'monospace',
      fontSize: '11px',
      color: '#8899aa',
      letterSpacing: 1
  ...
  drawMachinePanel() {
    ...
    this.add.rectangle(width / 2, height - 74, width, 148, 0x161b22);
    ...
    machines.forEach((m, i) => {
      ...
      const btn = this.add.rectangle(x, y, 104, 80, 0x1e2530).setInteractive();
      btn.machineKey = m.key;
      btn.machineColour = m.colour;

      this.add.rectangle(x, y, 104, 80).setStrokeStyle(1, 0x334455);
      ...
      btn.on('pointerout', () => {
        if (this.selectedMachine !== m.key) btn.setFillStyle(0x1e2530);
      });
    });
  }
  ...
  selectMachine(key, colour) {
    ...
    this.children.list.forEach(child => {
      if (child.machineKey) {
        child.setFillStyle(child.machineKey === key ? 0x2a3a4a : 0x1e2530);
      }
    });
  }
