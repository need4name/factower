...
this.add.text(x, y + 4, 'STORYLINE ' + (data.storyline || 1) + ' \xb7 LEVEL ' + (data.level || 1), {
fontFamily: 'monospace',
fontSize: '12px',
color: '#8899aa',
letterSpacing: 2
...
level:      1,
powerScore: 0,
// ── Starting materials: enough for exactly 2 Gunners ──────────────────
// Player must visit factory before first level.
// Rewards from levels replenish and grow the pool.
materials: {
plasticScrap:   2,   // 1 scrap = 1 Gunner
refinedPlastic: 0,
salvagedMetal:  0
},
// ── Starting stockpile: empty ─────────────────────────────────────────
// Player builds towers from materials — none given for free.
stockpile: {
gunner:    0,
bomber:    0,
barricade: 0
...
// currently in a combat mission. CombatScene flips this to false on entry
// and back to true on every exit path (BACK / VICTORY / DEFEAT).
// Foundation for the GDD's "factory frozen during waves" rule. Currently
// read but not acted on by FactoryScene; later milestones will gate
// worker/machine ticks on this value.
factoryActive: true,
createdAt: Date.now()
};
